...
void MessageEncoder::Encode(uint8_t* buffer, const sensor_avg_t* avg) 
{
    // Reset the checksum
    crc16.Reset();
    // Header
    ...
    buffer[index++] = PAYLOAD_SIZE; // Payload size == Message len
    // Accel X, Y, Z
    memcpy((buffer+index), &avg->a_x, sizeof(float));
    index +=sizeof(float);
    memcpy((buffer+index), &avg->a_y, sizeof(float));
    index +=sizeof(float);
    memcpy((buffer+index), &avg->a_z, sizeof(float));
    index +=sizeof(float);
    // Gyro X, Y, Z
    memcpy((buffer+index), &avg->g_x, sizeof(float));
    index +=sizeof(float);
    memcpy((buffer+index), &avg->g_y, sizeof(float));
    index +=sizeof(float);
    memcpy((buffer+index), &avg->g_z, sizeof(float));
    index +=sizeof(float);
    // Add CRC
    uint16_t checksum = crc16.AddBuffer(buffer, (HEADER_SIZE + PAYLOAD_SIZE));
    memcpy((buffer+index), &checksum, sizeof(uint16_t));
